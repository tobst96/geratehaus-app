...
- **Ein Login für alle** – die persönliche Anmeldung (`/mitglied/login`, E-Mail + Passwort) führt immer
  ...
  Klick **ohne erneute Passworteingabe** in den entsprechenden Bereich (eine standardmäßig aktive, in
  Einstellungen abschaltbare 2FA-Pflicht bleibt dabei als Schutzschicht bestehen – schlägt der
  Mailversand des Anmelde-Codes fehl, weicht sie auf einen konfigurierten Netzwerkdrucker aus, sonst
  bleiben Recovery-Codes der Ausweg). Namensauswahl + PIN bzw. Barcode bleiben ausschließlich dem
  Kiosk vorbehalten.
