...
async def konfliktvergleich(
    db: DbSession, _gruppenfuehrer: FahrzeugbuchungZugriff, buchung_id: int
) -> list[BuchungOut]:
...
async def genehmigen(db: DbSession, gruppenfuehrer: FahrzeugbuchungZugriff, buchung_id: int) -> BuchungOut:
    buchung = await buchung_service.get_buchung(db, buchung_id)
    if buchung is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Buchung nicht gefunden.")
    ergebnis = await buchung_service.genehmigen(db, buchung)
    await audit_service.protokolliere(
        db, gruppenfuehrer.name, "buchung_genehmigt", "buchung", buchung_id,
...
async def ablehnen(
    db: DbSession, gruppenfuehrer: FahrzeugbuchungZugriff, buchung_id: int, daten: BuchungAblehnen
) -> BuchungOut:
    buchung = await buchung_service.get_buchung(db, buchung_id)
    if buchung is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Buchung nicht gefunden.")
    ...
    await audit_service.protokolliere(
        db, gruppenfuehrer.name, "buchung_abgelehnt", "buchung", buchung_id,
