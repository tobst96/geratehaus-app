...
async def test_einstellungen_granular_geschuetzt(client, db):
    ...
    assert (await client.get("/api/v1/moderator/einstellungen", headers=h)).status_code == 403

    await berechtigungs_service.set_berechtigung(db, gf.id, "einstellungen", True)
    assert (await client.get("/api/v1/moderator/einstellungen", headers=h)).status_code == 200
...
async def test_update_ohne_einstellungen_403(client, db):
    ...
    assert (await client.get("/api/v1/moderator/update", headers=h)).status_code == 403
